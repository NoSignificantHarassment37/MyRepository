...
import { SistemaPedidosSchema, sistemaPedidosSchema } from "../Schemas/sistemaPedidos.schema";
import { Pedido } from "./pedido";
import { cantidadSchema } from "../Schemas/item.schema";
import {idSchema, nombreSchema, precioSchema, ProductoSchema} from "../Schemas/producto.shema";
...
export class SistemaPedidos{
    private autoIncrementable:number = 1;
    ...
    agregarProducto(nombre:string, precio:number):void{
        ...
        const id:number = this.autoIncrementable++;

        const productosCopia:Array<ProductoSchema> = [...this.productos, new Producto({
            id: id,
